...
export const PreferCall = () => {
    const { user, handleSignout } = useContext(UserContext);
    const [phonePreferences, setPhonePreferences] = useState([]);
    ...
    useEffect(() => {
        const fetchPhonePreferences = async () => {
            try {
                const response = await axios.get("https://grow-africa-api.vercel.app/contact/preference/phone");
                setPhonePreferences(response.data.data);
            } catch (error) {
                console.error("Error fetching phone Preferences:", error);
            }
        };

        fetchPhonePreferences();
    }, []);

    const exportData = () => {
        const csvContent = "data:text/csv;charset=utf-8,"
            + phonePreferences.map(preference => Object.values(preference).join(",")).join("\n");
        const encodedUri = encodeURI(csvContent);
        const link = document.createElement("a");
        link.setAttribute("href", encodedUri);
        link.setAttribute("download", "callPreference.csv");
            ...
            <Box className="lg:ml-64 lg:pl-4 lg:flex lg:flex-col lg:w-75% mt-5 mx-2">
                ...
                <Box className="bg-white rounded-xl p-4 shadow-md overflow-x-auto">
                    <Box className="px-4 py-2 text-left border-b-2 w-full">
                        <Typography variant="h5" className="font-bold text-yellow-600 inline">Preference: Phone Call</Typography> &emsp; &emsp;
                        <span>
                    ...
                    <table className="table-auto w-full">
                        ...
                        <tbody>
                            {phonePreferences.map((preference, index) => (
